# PST Testing Scenarios for Constant Contact Connector

## Part A: Authentication & Connectivity Verification
- **Scenario A1: Valid Credential Connect**: `connect_constant_contact_connector` saves token and validates endpoint.
...
- **Scenario B1: List Contacts**: `list_contacts` returns typed list with bounds.
- **Scenario B2: Get Contact**: `get_contact` returns entity details or 404 error.
- **Scenario B3: Health Audit**: `audit_contact_health` aggregates active records.

## Part C: Safe Write & Idempotency Testing
- **Scenario C1: Connection Lifecycle**: Clean disconnect via `disconnect_constant_contact_connector` without lingering secrets.
