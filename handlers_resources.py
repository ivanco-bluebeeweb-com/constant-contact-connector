...
async def list_contacts(params: ListContactParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({"contacts": items, "total": len(items)}, summary=f"Found {len(items)} contacts.")
...
async def get_contact(params: GetContactParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({"id": rid, "name": rname, "status": r.get("status"), "created_at": r.get("createdAt") or r.get("created_at"), "raw": r}, summary=f"Retrieved Contact {rid}.")
    except Exception as e:
